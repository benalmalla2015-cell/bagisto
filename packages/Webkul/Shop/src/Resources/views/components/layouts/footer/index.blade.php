    ...
    <div class="flex justify-between bg-[#F1EADF] px-[60px] py-3.5 max-md:justify-center max-sm:px-5">
        ...
        @if ($socialLinks->isNotEmpty())
            <div class="flex items-center gap-3">
                @if ($link = $socialLinks->get('whatsapp'))
                    <a href="{{ $link->url }}" target="_blank" rel="noopener" aria-label="WhatsApp" class="text-zinc-500 hover:text-green-500">
                        <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.127.558 4.122 1.532 5.855L0 24l6.335-1.498A11.93 11.93 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.818 9.818 0 01-5.007-1.374l-.36-.214-3.726.881.937-3.618-.235-.372A9.785 9.785 0 012.182 12C2.182 6.575 6.575 2.182 12 2.182S21.818 6.575 21.818 12 17.425 21.818 12 21.818z"/></svg>
                    </a>
                @endif
                @if ($link = $socialLinks->get('facebook'))
                    <a href="{{ $link->url }}" target="_blank" rel="noopener" aria-label="Facebook" class="text-zinc-500 hover:text-blue-600">
                        <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073C24 5.406 18.627 0 12 0S0 5.406 0 12.073C0 18.1 4.388 23.094 10.125 24v-8.437H7.078v-3.49h3.047V9.41c0-3.025 1.792-4.697 4.533-4.697 1.312 0 2.686.236 2.686.236v2.97h-1.513c-1.491 0-1.956.93-1.956 1.885v2.27h3.328l-.532 3.49h-2.796V24C19.612 23.094 24 18.1 24 12.073z"/></svg>
                    </a>
                @endif
                @if ($link = $socialLinks->get('instagram'))
                    <a href="{{ $link->url }}" target="_blank" rel="noopener" aria-label="Instagram" class="text-zinc-500 hover:text-pink-500">
                ...
                @endif
                @if ($link = $socialLinks->get('tiktok'))
                    <a href="{{ $link->url }}" target="_blank" rel="noopener" aria-label="TikTok" class="text-zinc-500 hover:text-black dark:hover:text-white">
                ...
                @endif
                @if ($link = $socialLinks->get('twitter'))
                    <a href="{{ $link->url }}" target="_blank" rel="noopener" aria-label="Twitter/X" class="text-zinc-500 hover:text-sky-500">
                        <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.746l7.73-8.835L1.254 2.25H8.08l4.259 5.632L18.244 2.25zm-1.161 17.52h1.833L7.084 4.126H5.117L17.083 19.77z"/></svg>
                    </a>
                @endif
            </div>
        @endif
